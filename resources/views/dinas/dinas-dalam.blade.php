...
<script>
    ...
    $(document).on('click', '.sp', function () {
        ...
        $.ajax({
            ...
            success:function(result){
                for (let i = 0; i < result.length; i++) {
                    checkbox += `<div class=" mb-2 form-check form-check-custom"><input class="form-check-input" name="pegawai_id[]" type="checkbox" value="`+result[i]['id_pegawai']+`" id="`+result[i]['id_dinas']+`a"/><label class="form-check-label" for="`+result[i]['id_dinas']+`a"> `+' '+result[i]['nama']+`</label></div>`;
                }
