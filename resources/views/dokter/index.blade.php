        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header py-7 d-flex justify-content-between">
                    <h2 class="modal-title">Tambah Dokter</h2>
            ...
            <div class="modal-content">
                ...
                <form action="{{route('obat.update')}}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-10">
                            <select class="form-control" name="id_poli" id="id_poli_u">
                                <option value="" disabled selected>Pilih Poli</option>
                                @foreach ($polis as $poli)
                                <option class="text-dark" value="{{ $poli->id }}">{{
...
<script>
    dt = $("#data_table").DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "{{ route('dokter.data') }}",
        "columns": [
            {data: 'DT_RowIndex', name: 'DT_RowIndex'},
            {data: 'nama', name: 'nama'},
            {data: 'alamat', name: 'alamat'},
            {data: 'no_hp', name: 'no_hp'},
            {data: 'id_poli', name: 'poli.nama_poli'},
            {
                data: 'action',     
                name: 'action', 
                orderable: true, 
    ...
    $(document).on('click', '.ubah', function () {
        ...
        let id_poli = $(this).data('id_poli')

        $('#id_u').val(id)
        $('#nama_u').val(nama)
        $('#alamat_u').val(alamat)
        $('#no_hp_u').val(no_hp)
        $('#id_poli_u').val(id_poli)
    });
